...
| image_count_score | 10 | Phase 4 enrichment의 `listing_details.image_count`/`listing_images` 우선, 없으면 대표 이미지 fallback |
| image_quality_proxy_score | 10 | enrichment image URL 또는 대표 image_url 기반 proxy |
| item_specifics_score | 10 | Phase 4 `listing_item_specifics` count 기반. 없으면 `needs_data` |
| shipping_score | 10 | Phase 4 shipping policy/handling time 우선, 없으면 `shipping_usd` fallback |
| return_policy_score | 5 | Phase 4 return policy 기반. 없으면 `needs_data` |
| category_score | 5 | Phase 4 category_id/category_name 기반. 없으면 `needs_data` |
...
불가능한 항목은 `null` 또는 `needs_data`로 처리한다. Migration 059 및 enrichment 실행 후에는 item specifics, image count, return/shipping policy, category score가 실제 데이터 기반으로 계산된다.
...
- `supabase/migrations/059_hermes_listing_enrichment.sql` 이후 enrichment cache 사용
...
`listing_details` 계열 enrichment cache가 없으면 기존 fallback 점수/needs_data로 동작한다.
