...
//     'all'                         → 모두
//
//   OPS-BRIEF-1A additions (owner-approved 2026-09-06):
//     exceptionType — whitelist filter for the auto-exception console
//                     drill-down. Only 'SKU_MATCH_FAILED' approved in 1A;
//                     new values require an explicit review step.
//     status=open   — semantic marker for "not done" (i.e. status != 'done').
//                     Aligns with briefing predicate so
//                     briefing.sku_match_failed == filtered detail total.
const ALLOWED_EXCEPTION_TYPES = ['SKU_MATCH_FAILED'];

router.get('/', async (req, res) => {
  try {
    const { status, scope, assigneeId, autoGenerated, exceptionType } = req.query;
    if (exceptionType !== undefined && exceptionType !== '' && !ALLOWED_EXCEPTION_TYPES.includes(exceptionType)) {
      return res.status(400).json({ error: 'invalid exceptionType' });
    }
