...
/** 활성 admin id 목록 — Phase 1: assignee_scope='operators' 자동 카드 수신자 해석에 사용 */
...
/**
 * dedupe_key 기준 활성 자동 카드 1개 조회.
 * 038 partial unique index (status != 'done' AND dedupe_key IS NOT NULL) 와 정합.
 * 없으면 null.
 */
async function findActiveByDedupeKey(dedupeKey) {
 ...
 *        recipient 링크 자체가 자동 카드를 자연 차단 (auto 카드는 operators 에게만 recipient 생성).
 *        단, admin 이 staff 에게 재배정한 자동 카드는 staff recipient 생김 → 정상 표시.
 * owner: 전체 task. 각 task에 aggregate {total, pending, in_progress, done} + recipients[] 포함.
 *
 * autoGenerated (admin path 만 적용):
 *   undefined / 'false' (default) → auto_generated=false 만 (사람 카드)
 *   'true'                        → auto_generated=true 만 (자동 예외 탭)
 *   'all'                         → 필터 없음
 *
 * OPS-BRIEF-1A (admin path 만 적용):
 *   exceptionType — server-side .eq('exception_type', value) 필터.
 *                   whitelist 는 라우트 계층에서 강제.
 *   status='open' — 'not done' 의미 (=신규 세만틱 마커). ops briefing 지표와 정합.
 */
async function listTasks({ user, status, scope, assigneeId, autoGenerated, exceptionType }) {
  const c = getClient();
  ...
  //   OPS-BRIEF-1A · status='open' 은 briefing 지표와 정합하는 not-done 세만틱.
  //   기존 status='pending'|'in_progress'|'done'|'blocked' 는 .eq 그대로 유지.
  if (status === 'open') {
    taskQ = taskQ.neq('status', 'done');
  } else if (status) {
    taskQ = taskQ.eq('status', status);
  }
  // autoGenerated 분기: 기본은 사람 카드만 (default-safe — 자동 예외는 별 탭)
  if (autoGenerated === 'true') {
    taskQ = taskQ.eq('auto_generated', true);
  } else if (autoGenerated !== 'all') {
    taskQ = taskQ.eq('auto_generated', false);
  }
  //   OPS-BRIEF-1A · exception_type whitelist 는 라우트 계층에서 이미 강제됨.
  //   여기서는 값이 있을 때만 서버-사이드 필터 적용 (count = detail 계약 유지).
  if (exceptionType) {
    taskQ = taskQ.eq('exception_type', exceptionType);
  }
  ...
  let items = tasks.map(t => {
    ...
    return {
      ...
      // 자동 예외 카드 필드 (2026-07-10) — 038 migration 확장 컬럼.
      // exceptionFilter.js 가 사용하는 값들. context 는 저장 시 이미 redact 통과.
      auto_generated: t.auto_generated,
      exception_type: t.exception_type,
      severity: t.severity,
      dedupe_key: t.dedupe_key,
      context: t.context || null,
      related_sku_id: t.related_sku_id,
...
async function createTask(taskValues) {
  ...
  } else if (taskValues.assignee_scope === 'operators') {
    recipientIds = await getActiveAdminIds();
 ...
 *
 * autoGenerated:
 *   undefined / 'false' (default) → 사람 카드 통계만 (운영 콘솔의 "오늘 업무" 위젯 기준)
 *   'true'                        → 자동 예외 카드만
 *   'all'                         → 모두
 */
async function getTodayStats({ ownerId, autoGenerated } = {}) {
  const now = new Date();
  ...
  if (autoGenerated === 'true') {
    q = q.eq('auto_generated', true);
  } else if (autoGenerated !== 'all') {
    q = q.eq('auto_generated', false);
  }
